...
describe("seed validation", () => {
  ...
  it("loads the expected row counts", () => {
    ...
    expect(seed.nutrientAmount).toHaveLength(58374);
    expect(seed.nutrientReference).toHaveLength(490);
  ...
  it("has exactly 23 nutrient rows for every food", () => {
    const seed = loadSeed();
    ...
    expect([...counts.values()].every((count) => count === 23)).toBe(true);
  });
});
