#!/usr/bin/env python3
"""
Append vitamin K to the frozen nutrient-amount seed, from the same official
MEXT source as the other seeded nutrients (八訂 増補2023, 第2章 本表).

  vitamin_k_ug   VITK   ビタミンK   (μg/日 basis, DRI 目安量 AI only, no UL)

Existing rows are never touched: the script appends only, and first re-reads
the existing nutrients for every food from the Excel to confirm the column
mapping still reproduces them exactly (fails loudly otherwise). Official
values are copied as-is and never recalculated (AGENTS.md).

Usage: python3 scripts/add_vitamin_k_from_mext.py [path/to/mext_ch2_honpyo.xlsx]
"""
import json
import re
import sys
...
NEW_NUTRIENT_ID = {"vitamin_k_ug": "VITK"}
...
def main():
    ...
    already = {code for _, code in ((row[0], row[1]) for row in compact)}
    ...
    for food in food_master:
        excel_row = by_code.get(food["official_food_code"])
        if excel_row is None:
            raise SystemExit(f"no Excel row for {food['food_id']}")
